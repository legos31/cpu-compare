        ...
        <div class="row" id="gpu">
            <?php foreach ($cpus as $cpu) : ?>
                <div class="col-xl-3 col-lg-4 col-sm-6">
                    <div class="review">
                        <img loading="lazy" src="<?= $cpu->image_logo ?>">
                        <span><?= $cpu->name ?></span>
                        <a href="<?= Url::to(['gpu/index', 'alias' => $cpu->alias]) ?>" class="btn btn-orange"><?=Yii::t('app', 'View')?></a>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
    ...
    $.get('/' + id + '/reviews?page=' + k, (res) => {
        ...
        for(let card of res){
            ...
            let review = createElem('div', '', {class: 'review'})
                ...
                createElemIn(review, 'a', 'View', {class: 'btn ' + (card.category_id == 1 ? 'btn-orange' : 'btn-orange'), href: '/'+(card.category_id == 1 ? 'cpu' : 'gpu')+'/'+card.alias})
            add(col, review)
            $('#'+id).append(col)
        }
    })
