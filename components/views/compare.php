...
<div class="sidebar-compare">
    ...
    <?php if ($compare && (count($compare) > 1)) {
        foreach ($compare as $item) : ?>
            ...
            <?php if ($card1->id != $card->id) : ?>
                <a href="<?= Url::to(['cpu/compare', 'vs' => $card1->alias . '-vs-' . $card2->alias]) ?>" class="sidebar-compare-item"> 
    ...
    <?php } else {
        foreach ($similar as $item) : ?>
            <a href="<?= Url::to(['cpu/compare', 'vs' => $card->alias . '-vs-' . $item->alias]) ?>" class="sidebar-compare-item">
